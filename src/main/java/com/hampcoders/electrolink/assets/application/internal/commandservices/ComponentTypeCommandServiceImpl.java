...
/**
 * Component type command service implementation.
 */
@Service
public class ComponentTypeCommandServiceImpl implements ComponentTypeCommandService {
  ...
  /**
   * Constructs a ComponentTypeCommandServiceImpl.
   *
   * @param componentTypeRepository The component type repository.
   * @param componentRepository The component repository.
   *
   */
  public ComponentTypeCommandServiceImpl(final ComponentTypeRepository componentTypeRepository,
  ...
  @Override
  public ComponentTypeId handle(final CreateComponentTypeCommand command) {
    ...
    final var componentType = new ComponentType(command);
    final var savedComponentType = componentTypeRepository.save(componentType);
    return new ComponentTypeId(savedComponentType.getId());
  }

  @Override
  public Optional<ComponentType> handle(final UpdateComponentTypeCommand command) {
  ...
  @Override
  public Boolean handle(final DeleteComponentTypeCommand command) {
    final Long typeId = command.componentTypeId();

    if (!componentTypeRepository.existsById(typeId)) {
      return false;
    }

    final ComponentTypeId typeIdValueObject = new ComponentTypeId(typeId);

    if (componentRepository.existsByComponentTypeId(typeIdValueObject)) {
      throw new IllegalStateException("Cannot delete component type: it is currently "
          + "in use by one or more components.");
    }
