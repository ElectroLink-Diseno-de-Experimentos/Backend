...
/**
 * Implementation of the command service for the TechnicianInventory aggregate.
 */
@Service
public class TechnicianInventoryCommandServiceImpl implements TechnicianInventoryCommandService {
  ...
  /**
   * Constructs a TechnicianInventoryCommandServiceImpl.
   *
   ...
   */
  public TechnicianInventoryCommandServiceImpl(
  ...
  @Override
  public UUID handle(final CreateTechnicianInventoryCommand command) {
    if (technicianInventoryRepository.existsByTechnicianId(command.technicianId().technicianId())) {
      throw new IllegalStateException(
          "Technician inventory already exists for this technician ID");
    }
  ...
  @Override
  public Optional<TechnicianInventory> handle(final AddComponentStockCommand command) {
    var inventory = technicianInventoryRepository.findByTechnicianId(
        command.technicianId()).orElseThrow(() -> new EntityNotFoundException(
        "TechnicianInventory not found for technician ID: " + command.technicianId()));

    var component = componentRepository.findByComponentUid(command.componentId())
        .orElseThrow(() -> new EntityNotFoundException(
            "Component not found with UID: " + command.componentId()));
  ...
  @Override
  @Transactional
  public Optional<TechnicianInventory> handle(final UpdateComponentStockCommand command) {
    ...
    TechnicianInventory inventory = technicianInventoryRepository.findByTechnicianId(technicianId)
        .orElseThrow(() -> new EntityNotFoundException(
            "TechnicianInventory not found for technician ID: " + technicianId));
    ...
    if (stockOpt.isEmpty()) {
      throw new EntityNotFoundException(
          "Component not found in technician's inventory: " + command.componentId());
    }
  ...
  @Override
  public Boolean handle(final DeleteComponentStockCommand command) {
    Long technicianId = command.technicianId();
